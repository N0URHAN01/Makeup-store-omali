...
<div class="max-w-2xl mx-auto bg-white p-8 shadow-lg rounded-2xl border border-gray-200">
    ...
    {{-- HEADER --}}
    <h1 class="text-3xl font-bold text-gray-900 mb-6">Edit Governorate</h1>
    ...
    {{-- FORM --}}
    <form action="{{ route('admin.governorates.update', $governorate->id) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="number" name="shipping_cost" step="0.01" 
                   class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-pink-300"
                   value="{{ old('shipping_cost', $governorate->shipping_cost) }}" required>
            @error('shipping_cost') 
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p> 
            @enderror
        </div>
